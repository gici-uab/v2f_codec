...
class OutputBitStream:
    ...
    def put_bits(self, bits, pad_to_bytes):
        """Put a list of bit values. If pad_to_bytes is True, then 
        bits are padded to an integer byte length (with zeros).
        """
        assert all(b == 0 or b == 1 for b in bits), bits
        self.pending_bits.extend(bits)
        if pad_to_bytes and len(bits) % 8 != 0:
            extra_bits = 8 - (len(bits) % 8)
            self.pending_bits.extend([0]*(extra_bits))
        if len(self.pending_bits) > self.pending_bit_buffer_size:
            self.flush_complete_bytes()
    ...
    def flush_complete_bytes(self):
        ...
        if not all(b == 0 or b == 1 for b in self.pending_bits):
            print("[watch] self.pending_bits = {}".format(self.pending_bits))
...
class InputBitStream:
    ...
    def get_bits(self, n_bits):
        assert n_bits >= 1
        return [self.get_bit() for _ in range(n_bits)]
    ...
    def skip_bits(self, n_bits=1):
        """Advance the next-bit index by one. Don't make any extra checks.
        """
        assert n_bits >= 0
        self.current_bit_position += n_bits
