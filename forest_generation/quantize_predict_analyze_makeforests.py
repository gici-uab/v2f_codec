...
def analyze_prediction_results(qstep_to_df):
    ...
    for q, full_df in qstep_to_df.items():
        ...
        def keep_64_symbols(d):
            new_d = collections.defaultdict(int)
            new_d.update({k:v for k,v in d.items() if k < 65})
            return {k:new_d[k] for k in range(65)}
        dna = enb.aanalysis.DictNumericAnalyzer(combine_keys_callable=keep_64_symbols)
        ...
        dna.get_df(
            ...
            column_to_properties=banded_image_analysis.SatellogicRegionsTable.column_to_properties,
            group_by="version_name",
            # key_to_x={k:i for i,k in enumerate(banded_image_analysis.analyzed_block_sizes)},
            fig_width=10,
            x_tick_label_angle=90)
...
if __name__ == '__main__':
    ...
    # options.base_dataset_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "satellogic_1")
